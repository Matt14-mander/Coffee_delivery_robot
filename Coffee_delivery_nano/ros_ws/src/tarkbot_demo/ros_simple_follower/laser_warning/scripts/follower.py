...
from laser_warning.msg import position as PositionMsg
from std_msgs.msg import String as StringMsg
from std_msgs.msg import Int8
from laser_warning.cfg import PID_reconfigConfig
from std_msgs.msg import Int8
# PID的参数
KP=[1.2, 0]
KI=[0, 0]
...
class Follower:
	def __init__(self):
		...
		self.warning_angle = 70
		self.warning_Dist = 0.5
		...
		self.beepPublisher = rospy.Publisher('/beep', Int8, queue_size =3)
        # 
	...
	def dynamic_reconfigure_callback(self,config,level):
		...
		self.warning_angle = config.warningAngle
		self.warning_Dist = config.warningDist
		return config
	...
	def positionUpdateCallback(self, position):
		...
		# 如果距离大于报警距离或者角度大于报警角度则进行警报。
		if self.i < 10:
			self.i = self.i +1
		elif self.i == 10:		#语音识别标志
			self.publish_flag()
			self.i = 11	
		if (abs(angleX) > ((3.14/180)*self.warning_angle)) or (distance > self.warning_Dist):
			beep = 0
			velocity = Twist()
			velocity.linear = Vector3(0.,0.,0.)
			velocity.angular= Vector3(0.,0.,0.)
			self.cmdVelPublisher.publish(velocity)
			self.beepPublisher.publish(beep)
			return
...
class simplePID:
	...
	def __init__(self, target, P, I, D):
		...
		rospy.loginfo('PID initialised with P:{}, I:{}, D:{}'.format(P[0],I[0],D[0]))
		self.Kp		=np.array(P)
		self.Ki		=np.array(I)
		self.Kd		=np.array(D)
...
if __name__ == '__main__':
	print('starting')
	laserfwflagPublisher = rospy.Publisher('/laser_waring_flag', Int8, queue_size =1)
	rospy.init_node('warnning')
	follower = Follower()
	try:
		rospy.spin()
