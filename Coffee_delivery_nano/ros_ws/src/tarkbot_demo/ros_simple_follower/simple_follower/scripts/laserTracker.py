...
class laserTracker:
	def __init__(self):
                ...
                self.scanSubscriber = rospy.Subscriber('/scan', LaserScan, self.registerScan) # 接收/scan话题进入回调函数
                self.positionPublisher = rospy.Publisher('/object_tracker/current_position', PositionMsg,queue_size=3) # 发布当前跟随的障碍物角度和距离
	...
	def registerScan(self, scan_data):
			...
			for i in sortedIndices:
				...
				with np.errstate(invalid='ignore'):
					...
					# this will also be false for all tempMinDistance = NaN or inf
						...
						minDistanceID = i
						minDistance = ranges[minDistanceID]   # 获得最近障碍物的距离
		...
		if(minDistance > scan_data.range_max):
			...
			# calculate angle of the objects location. 0 is straight ahead 获得最近障碍物的角度
                        minDistanceAngle = scan_data.angle_min + minDistanceID * scan_data.angle_increment
                        if (minDistanceAngle < (-3.14)) : 
			            #更新角度数据
                            minDistanceAngle = minDistanceAngle + 6.2830
                        else:
			            # 与目标之间的角度
                            minDistanceAngle = minDistanceAngle
			# here we only have an x angle, so the y is set arbitrarily    发布障碍物角度距离信息
