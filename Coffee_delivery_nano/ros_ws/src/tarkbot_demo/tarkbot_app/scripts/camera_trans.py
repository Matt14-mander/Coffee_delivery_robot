...
class Image_converter:
 ...
 def callback(self, data): #订阅者接受到消息后的回调函数，用于处理压缩的图片数据
    ...
    numpy_arr = numpy.frombuffer(data.data, numpy.uint8)
    image_numpy = cv2.imdecode(numpy_arr, cv2.IMREAD_COLOR)

    # 创建要发布的图片信息
    # 三个变量内容：header,format,data. data是cv2的图片转换成numpy.array，再输出成字符串
    trans_img = CompressedImage()
    trans_img.header.stamp = rospy.Time.now()
    trans_img.format = "jpeg"
    trans_img.data = numpy.array(cv2.imencode('.jpg', image_numpy)[1]).tostring()

     #发布消息
    self.image_pub.publish(trans_img)
