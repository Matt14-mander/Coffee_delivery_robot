...
#include <xf_mic_asr_offline/Lidar_Angle.h>
...
/**************************************************************************
函数功能：底盘控制器标志位sub回调函数
入口参数：far_away_flag_msg  command_recognition.cpp
返回  值：无
**************************************************************************/
void far_away_flag_Callback(std_msgs::Int8 msg)
{
	far_away_flag = msg.data;
...
/**************************************************************************
函数功能：远离障碍物控制部分(akm车型)
入口参数：angle
返回  值：无
**************************************************************************/
void far_away_move_akm()
{
    ...
    lidar_angle.start_angle = 90;
	lidar_angle.end_angle = 270;
    lidar_angle_set.publish(lidar_angle);
	temp_number = 50;
	...
	if((distance1>(radar_range+0.3)))
	{
       	far_away_flag = 0;
		temp_number=30;
		ticks = 0;
		lidar_angle.start_angle = 130;
	    lidar_angle.end_angle = 230;
        lidar_angle_set.publish(lidar_angle);
...
void far_away_move()
{
	...
	cmd_vel_Pub.publish(cmd_vel_msg);
    lidar_angle.start_angle = 30;
	lidar_angle.end_angle = 330;
    lidar_angle_set.publish(lidar_angle);
...
void follow_turn(int angle)
{
	...
	if((angle<=150)||(angle>330))
	{
		...
		turn_angle = -(330 - angle);
	}
	else
	{
		//angle=360-angle;
		cmd_vel_msg.angular.z = -angular_turn_msg;
		turn_angle = -(angle - 330);
	}
	
	angular_duration = turn_angle / angular_turn_msg /180 * 3.14159;    //计算转弯时间
	printf("time =%f\n",angular_duration);
	ticks = int(angular_duration * rate1);    //计算控制次数
	printf("ticks =%d\n",ticks);

	/***控制转向***/
	for(int i = 0; i < ticks + 15 ; i++)
	{
...
{
	if(cmd_vel_msg.linear.x > 0 && ((dis_angleX < 1.0 && dis_angleX > 0) || (dis_angleX > -1.0 && dis_angleX < 0)))
		return 1;
...
int main(int argc, char** argv)
{
	...
	/***创建远离障碍物话题订阅者***/
	ros::Subscriber far_away_flag_sub = node.subscribe("far_away_flag", 1, far_away_flag_Callback);
  ...
  /***创建障碍物方位话题订阅者***/
    ...
    lidar_angle_set = node.advertise<xf_mic_asr_offline::Lidar_Angle>("lidar_angle_set",1);

	ros::Subscriber goal_reach_sub = node.subscribe("/move_base/result", 1, goal_reach_Callback);

	node.param("/motion_control/audio_path", audio_path, std::string("~/catkin_ws1111/src/xf_mic_asr_offline"));
