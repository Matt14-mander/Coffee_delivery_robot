...
from tarkbot_yolov5.msg import Image_Data
...
class msgToimg:
    def __init__(self):
        ...
        self.image_sub = rospy.Subscriber("camera/image_msg", Image_Data, self.image_sub_callback)
        self.image_sub = rospy.Subscriber("/camera/image_raw", Image_Data, self.image_raw_callback)
        self.pub_img = rospy.Publisher("yolov5/image", Image, queue_size=10)
        self.pub_img_raw = rospy.Publisher("/image_raw", Image, queue_size=10)

    def image_sub_callback(self,data):
        if not isinstance(data, Image_Data): return
        # 将自定义图像消息转化为图像
