...
void  ToLaserscanMessagePublish(ldlidar::Points2D& src, ldlidar::LiPkg* commpkg, LaserScanSetting& setting, ros::Publisher& lidarpub) {
  ...
  std::sort(tmp.begin(), tmp.end(), [](ldlidar::PointData a, ldlidar::PointData b) { return a.angle < b.angle; });
  // Adjust the parameters according to the demand
  angle_min = ANGLE_TO_RADIAN(tmp.front().angle);
  angle_max = ANGLE_TO_RADIAN(tmp.back().angle);
  range_min = 0.02;
  range_max = 12;
