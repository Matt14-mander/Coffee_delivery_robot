...
SYSTEM_PROMPT_CATCH_2 = '''
我即将说一句给机械臂的指令，你帮我从这句话中提取出需要抓取的物体，并从这张图中分别找到这个物体左上角和右下角的像素坐标，输出json数据结构。

例如，如果我的指令是：请帮我去取一杯咖啡。
你输出这样的格式：
{
 "object":"咖啡",
 "object_xyxy":[[102,505],[324,860]],
}
...
def yi_vision_api(PROMPT='帮我把红色方块放在钢笔上', img_path='temp/vl_now.jpg', vlm_option=0):
    ...
    if vlm_option==0:
        ...
        SYSTEM_PROMPT=SYSTEM_PROMPT_CATCH_2
...
def post_processing_viz_2(result, img_path, check=False):
    ...
    # 左上角像素坐标
    OBJECT_X_MIN = int(result['object_xyxy'][0][0] * img_w / FACTOR)
    OBJECT_Y_MIN = int(result['object_xyxy'][0][1] * img_h / FACTOR)
    # 右下角像素坐标
    OBJECT_X_MAX = int(result['object_xyxy'][1][0] * img_w / FACTOR)
    OBJECT_Y_MAX = int(result['object_xyxy'][1][1] * img_h / FACTOR)
    # 中心点像素坐标
    ...
    # 可视化
    # 画物体框
    img_bgr = cv2.rectangle(img_bgr, (OBJECT_X_MIN, OBJECT_Y_MIN), (OBJECT_X_MAX, OBJECT_Y_MAX), [0, 0, 255], thickness=3)
    # 画中心点
    img_bgr = cv2.circle(img_bgr, [OBJECT_X_CENTER, OBJECT_Y_CENTER], 6, [0, 0, 255], thickness=-1)
    # 写中文物体名称
    img_rgb = cv2.cvtColor(img_bgr, cv2.COLOR_BGR2RGB) # BGR 转 RGB
    img_pil = Image.fromarray(img_rgb) # array 转 pil
    draw = ImageDraw.Draw(img_pil)
    # 写物体中文名称
    draw.text((OBJECT_X_MIN, OBJECT_Y_MIN-32), OBJECT_NAME, font=font, fill=(255, 0, 0, 1)) # 文字坐标，中文字符串，字体，rgba颜色
    img_bgr = cv2.cvtColor(np.array(img_pil), cv2.COLOR_RGB2BGR) # RGB转BGR
    # 保存可视化效果图
    cv2.imwrite('temp/vl_now_viz.jpg', img_bgr)
